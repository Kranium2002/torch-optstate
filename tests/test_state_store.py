...
def test_state_store_basic():
    ...
    store.commit(param, state, codecs)
    
    materialized = store.materialize(param)
...
def test_state_store_multiple_params():
    ...
    store.commit(p1, {'v': torch.randn(10)}, {'v': FP32Codec()}) # 40 bytes
    store.commit(p2, {'v': torch.randn(20)}, {'v': FP32Codec()}) # 80 bytes
    
    assert store.get_memory_usage() == 120
    
    store.commit(p1, {'v': torch.randn(10)}, {'v': FP16Codec()}) # 20 bytes
